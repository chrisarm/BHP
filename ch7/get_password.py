...
def get_a_password(message=message):
    ...
    try:
        ...
        while (
            ...
            password_pass = [len(password) >= min_pass_length]
            password_pass.append(re.search(r'[a-z]', password))
            password_pass.append(re.search(r'[A-Z]', password))
            password_pass.append(re.search(r'[0-9]', password))

            if not all(password_pass):
                print(
