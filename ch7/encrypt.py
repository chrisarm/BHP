...
def derive_key(password, salt_file_path='custom.salt', test=False):
    ...
    try:
        if not Path(salt_file_path).exists():
            raise ValueError('Salt file path not found: {}'.format(salt_file_path))
        else:
...
DECYPTION: python3 encrypt.py -f <file_name> -d
    ...
    if not action_args.salt and (not action_args.file or not Path(action_args.file).exists()):
    ...
    if action_args.salt:
        ...
        if salt:
            ...
            raise RuntimeError('Problem while trying to write to the custom salt file')


    try:
        password = ''
        if (  # Ensure provided password of string type
